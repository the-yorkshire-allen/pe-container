...
- **FR-007**: The system MUST validate the presence and integrity of the required persisted state during startup and surface a clear status when the state is missing, incomplete, corrupted, or incompatible with the image being launched. Integrity validation MUST include, at minimum: required lifecycle marker files exist and are parseable, persisted version metadata is well-formed and comparable to runtime image metadata, and required persisted paths are present and writable.
...
- **FR-011a**: After a normal restart, the system MUST expose an operator-visible continuity signal for previously connected nodes (for example last check-in status/timestamp or equivalent node-state evidence) so restart-time node manageability can be verified.
...
- **SC-001b**: In 100% of invalid-build-input scenarios (missing version, non-absolute or unreadable local tar.gz file path, or unusable installer artifact), the build fails fast with actionable operator-facing error messaging.
...
- **SC-005**: In 100% of defined restart continuity validations, node-manageability continuity metrics (for example agent check-in and post-restart management operations) are captured and compared pre-restart vs post-restart through an external validation harness, then reported alongside in-container `/status/v1/simple` smoke results.
